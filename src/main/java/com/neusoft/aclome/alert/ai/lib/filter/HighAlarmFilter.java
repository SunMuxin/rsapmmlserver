...
public class HighAlarmFilter implements Filter{
		...
		try {
			...
			while(solr_data.hasNextFacet()) {
				...
				for (Map.Entry<String, JsonElement> entry : info.entrySet()) {
					...
					alarm_object.addProperty("highAlarm", entry.getValue().getAsDouble());
			...
			Util.error("HighAlarmFilter", e.getMessage());
		}
	}
}
