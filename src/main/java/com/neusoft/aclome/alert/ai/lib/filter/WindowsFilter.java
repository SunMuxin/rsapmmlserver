...
public class WindowsFilter implements Filter{

	@Override
	public void filter(Object in, Object out) {
		// TODO Auto-generated method stub
		if (!((JsonObject) in).has("WIN_CpuUsage")) return ;
		if (!((JsonObject) in).has("WIN_DiskUsePercent")) return ;
		if (!((JsonObject) in).has("WIN_MemoryUsage")) return ;
		if (!((JsonObject) in).has("res_id")) return ;
		
		String res_id = ((JsonObject) in).get("res_id").getAsString();
		double CpuUsage = ((JsonObject) in).get("WIN_CpuUsage").getAsDouble();
		double DiskUsePercent = ((JsonObject) in).get("WIN_DiskUsePercent").getAsDouble();
		double MemoryUsage = ((JsonObject) in).get("WIN_MemoryUsage").getAsDouble();
