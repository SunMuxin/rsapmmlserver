...
import com.realsight.westworld.server.context.JmThreadInfoToSolr;
...
public class JmThreadADModel extends Thread{
	...
	private static Logger logger = (Logger) LoggerFactory.getLogger(JmThreadADModel.class);
	private final String JMThread_URL;
	private static final Double th = 0.75;
	private Double pre_facet_value = 0.0;
	
	public JmThreadADModel(String JMThread_URL,
			String SOLR_URL, 
			Long timestamp,
			String ad_name,
		...
		if (entrys!=null){
			...
			for (Entry<String, Double> entry : entrys.getFirst()) {
				try {
					...
					System.out.println("jm_thread " + res + " " + facet_value + " " + TimeUtil.formatUnixtime2(entrys.getSecond()));
					if (res == null) continue;
					if (!Double.isNaN(facet_value)) {
						pre_facet_value = facet_value;
					}
					new Thread(new JmThreadInfoToSolr(iterator,
							JMThread_URL,
							pre_facet_value,
							entrys.getSecond(),
