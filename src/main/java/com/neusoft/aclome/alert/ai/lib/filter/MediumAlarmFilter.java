...
public class MediumAlarmFilter implements Filter{
		...
		try {
			...
			while(solr_data.hasNextFacet()) {
				...
				for (Map.Entry<String, JsonElement> entry : info.entrySet()) {
					...
					alarm_object.addProperty("mediumAlarm", entry.getValue().getAsDouble());
			...
			Util.error("MediumAlarmFilter", e.getMessage());
		}
	}
}
