...
public class DBHealthFilter implements Filter{

	@Override
	public void filter(Object in, Object out) {
		// TODO Auto-generated method stub
		if (!((JsonObject) in).has("dbHealth")) return ;
		if (!((JsonObject) in).has("res_id")) return ;

		String res_id = ((JsonObject) in).get("res_id").getAsString();
		double health_value = ((JsonObject) in).get("dbHealth").getAsDouble();
		...
		if (res_object.has("dbHealth")) {
			health_object = res_object.get("dbHealth").getAsJsonObject();
		...
		res_object.add("dbHealth", health_object);
